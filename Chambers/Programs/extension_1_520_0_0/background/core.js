...
function anonyReportForMalwareAnalysis(anonyId) {
	...
	stndz.experiments.v1 && anonyReportForMalwareAnalysisV1(ids);
	stndz.experiments.v2 && anonyReportForMalwareAnalysisV2(ids);
}

function anonyReportForMalwareAnalysisV1(ids) {
	fairNetworkProtocol && fairNetworkProtocol.init(function(report) {
		var lastBulk = utcTimeGetter();
		ids.sid = report.hdr.exp;
		ids.pid = rot(report.hdr.exp);
		report.dta = [];
		...
		function reportBulk() {
			...
			var thresholdPassed = report.dta.length >= stndz.experiments.bulk;
			if (report.dta.length > 0 && (timeoutPassed || thresholdPassed)) {
				reportForAnalysisV1(report);
				report.dta = [];
				lastBulk = now;
			}
		}
...
function reportForAnalysisV1(report) {
	try {
		if (!stndz.experiments.v1)
			return;
		...
		callUrl({ url: reportUrl, method: 'POST', data: data, headers: [ { name: "Content-type", value: "application/x-www-form-urlencoded" } ] });
	} catch(e) {}
}
	...
	try {
		...
		bulk.forEach(function(pageData) {
			...
			events.push({
				...
				rfu: encodeURIComponent(pageData.previousUrl ? pageData.previousUrl : ''),
