...
*/
	for (var j = 0; j < 4; j++) {
	    for (var i = 0; ;) {
	        var o = FindFirstUrl(html, j == 3 ? ".m3u8" :j == 2 ? ".mov" : j == 1 ? ".flv" : ".mp4", i);
	        if (!o || !o.start)
	            break;
	        i = o.start;
			
	        allUrlsList.push({ 'url': o.mp4, 'title': document.title, 'type': j==3?"m3u8":"video" });
	    }
	}
...
function OnAddToVideoList(info, play) {
    ...
    chrome.runtime.sendMessage({ msg: "OnSP24AddVideo", info: info, url: location.href, play: play }, function (response) { 
				if (typeof (response) == 'undefined' && chrome.runtime.lastError)
					return;
			});
}
